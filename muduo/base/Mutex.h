...
public:
  ...
  /**
   * @brief 断言已经被此线程占有，可用于在cond.wait()前进行断言
   *
   */
   ...
   *
   */
  void lock() ACQUIRE() { //仅供MutexLockGuard调用
   ...
   *
   */
  void unlock() RELEASE() { //仅供MutexLockGuard调用
    unassignHolder();
    MCHECK(pthread_mutex_unlock(&mutex_));
  }

  /**
   * @brief 返回内部持有的mutex的指针
   *
