...
struct SlotImpl : boost::noncopyable
{
  ...
  ~SlotImpl()
  {
    ...
    if (data)
    {
      data->clean();class StockFactory :public boost::enable_shared_from_this<StockFactory>,
                    boost::noncopyable
{
 public:
  boost::shared_ptr<Stock> get(const std::string& key)
  {
    ...
    boost::weak_ptr<Stock>& wkStock=stocks_[key];//wkStock为引用
    pStock=wkStock.lock();
    if(!pStock)
    {
        pStock.reset(new Stock(key),
        boost::bind(&StockFactory::weakDeleteCallback,
        //转型为weak_ptr 才不会延长生命期（bind拷贝的是实参）
 ...
 private:
  void weakDeleteCallback(const boost::weak_ptr<StockFactory>& wkFactory,
                          Stock* stock)
  {
      boost::shared_ptr<StockFactory> factory(wkFactory.lock());//尝试提升
      if(factory) {//如果factory还在，就清理stocks_
